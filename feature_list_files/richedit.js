...
//---------------------------------------------------------------------------------------------------------
  ...
  var txtHeading         = "Heading";
  var txtClearFormatting = "Clear Formatting";
  var txtJustifyLeft     = "向左對齊";
  var txtJustifyCenter   = "置中";
  var txtJustifyRight    = "向右對齊";
  var txtJustifyFull     = "左右對齊";
  var txtOrderedList     = "編號清單";
  var txtUnorderedList   = "項目符號";
  var txtOutdent         = "減少縮排";
  var txtIndent          = "增加縮排";
  var txtInsertHR        = "加入水平分隔線";
  var txtInsertTable     = "加入表格";
  var txtInsertGraph     = "加入圖表";
  var txtInsertBullet    = "Insert Bullet Point";
  var txtInsertImage     = "加入圖片";
  var txtInsertText      = "在這裡輸入文字";
  var txtFont            = "字型";
  var txtSize            = "文字大小";
  var txtBold            = "粗體";
  var txtItalic          = "斜體";
  var txtUnderline       = "底線";
  var txtFontColor       = "文字顏色";
  var txtBGColor         = "文字背景顏色";
  var txtHyperlink       = "連結";
  var txtCut             = "剪下";
  var txtCopy            = "複製";
  var txtPaste           = "貼上";
  ...
  var txtRedo            = "重做";
  var txtBorder          = "Border";
  var txtBorderColor     = "框線顏色";
  var txtCellColor       = "欄位背景顏色";
  var txtCellSpacing     = "框線與欄位間留白";
  var txtCellPadding     = "欄位與文字間留白";
  var txtColumns         = "欄數";
  var txtRows            = "列數";
  var txtCreate          = "確定";
  var txtCancel          = "取消";
  ...
  var txtViewValues      = "View Values";
  var txtLegend          = "Legend";
  var txtViewSource      = "切換HTML程式碼編輯器";
  var txtViewEditor      = "切換圖文顯示編輯器";
  var gogoday = "";
function EDITOR() {
//---------------------------------------------------------------------------------------------------------
// Configuration
//---------------------------------------------------------------------------------------------------------
  ...
  this.textFontColor = '#666';                       // text field font color
  
  this.setFocus = false;                        // focus text field on load (true or false)
  this.fieldName = "richEdit";                  // default field name
  this.iconPath = "/include/javascript/editor/richedit/icons";                      // path to icons
...
//---------------------------------------------------------------------------------------------------------
  ...
  this.getEditor = function() {
    var e = false;
    if(GK){
    	e = document.getElementById('rtoIFrame' + this.id).contentWindow;
    }else if(IE){
    	e = document.getElementById('rtoIFrame' + this.id).contentWindow;
    }
    if(e && !DM) e = false;
  ...
  //alert(content.replace(/<STYLE>[^<]+<\/STYLE>(\r?\n)*/gi, ''));
    if(this.editor = this.getEditor()) {
      ...
      if(GK) {
          this.editor.document.execCommand('useCSS', false, false);
      }
      this.editor.document.open();
      //alert(this.wordWrap(html));
  ...
  this.doCmd = function(cmd, opt) {
    ...
    if(cmd && opt) {
      if(cmd == 'insertHTML' && IE){ 
      	this.curSelection.pasteHTML(opt);
      }else{ 
      	this.fnExec(cmd, opt);
      }
    }else if(cmd){
  ...
  this.insertImage = function() {
	...
	this.setFocus = true;
    document.fImage.id.value = this.id;
	var top = (screen.availHeight-500)/2-20;
	var left = (screen.availWidth-400)/2;
	window.open(this.urlInsertImg, '', 'height=250, width=500, top='+top+', left='+left+', scrollbars=yes, resizable=yes, status=yes');
  }
  this.creatImageFromUpload = function(src, pic_position, json_size) {
		this.setFocus = true;
		if(src){
	      var html = '<p><img src="' + src +'"';
	      var suffixHTML = '';
		  var htmlAlign = '';
		    switch(pic_position){
			    case 1 :
				      html += ' style="float:right; margin-left:0.5em;"';
				      suffixHTML = '請在此輸入您的使用心得內容';
					  htmlAlign = ' align="right"';
				  break;
			    case 2 :
				      html += 'style="float:left; margin-right:0.5em;"';
				      suffixHTML = '請在此輸入您的使用心得內容';
					  htmlAlign = ' align="left"';
				  break;
			    case 3 :
				      //html += 'margin:auto; display:block; text-align:center;';
			    	//edit by jonathan @ 20100927 for text align.
					  htmlAlign = ' style="margin:0; display:block; text-align:left" ';
				  break;
		  }
		  
		  var htmlWidth = '';
		  var htmlHeight = '';
		  var newSize = json_size;
		  //return false;
		  if(newSize.height){
			 //html += 'height:'+newSize.height+'px;';
			 htmlHeight = ' height="'+newSize.height+'"';
		  }
		  if(newSize.width){
		      //html += 'width:'+newSize.width+'px;';
		      htmlWidth = ' width="'+newSize.width+'"';
		  }
		  html += '"'+htmlAlign+htmlWidth+htmlHeight; 
		  
		  html += ' />'+suffixHTML+'</p><br /><div style="clear:both;"></div><br />';
  ...
  this.buildEditor = function() {
	...
	document.writeln('<td>在文章中&nbsp;</td><td class="cssRaised' + this.id + '" widt="20" height="20" onMouseOver="this.className=\'cssPressed' + this.id + '\'" onMouseOut="this.className=\'cssRaised' + this.id + '\'" onMouseDown="this.className=\'cssPressed' + this.id + '\'" onMouseUp="this.className=\'cssRaised' + this.id + '\'" onClick="rto[' + this.id + '].insertImage()" title="' + txtInsertImage + '"><img src="' + this.iconPath + '/image.gif" width="68" height="16" align="absmiddle" /></td>');
    ...
    document.writeln('<center><input style="border-right: 1px solid #ccc;border-bottom: 1px solid #ccc;" onMouseOver="this.className=\'cssPressed' + this.id + '\'" onMouseOut="this.className=\'\'" type=button id="rtoButton' + this.id + '" value="' + txtViewSource + '" onClick="rto[' + this.id + '].toggleSource()"></center>');
